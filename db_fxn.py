...
def create_table():
    c.execute('''CREATE TABLE IF NOT EXISTS taskstable(task TEXT, task_status TEXT,
              task_due_date DATE)''')

def add_tasks(task, task_status, task_due_date):
    c.execute('''INSERT INTO taskstable(task, task_status,
              task_due_date) VALUES(?,?,?)''',(task, task_status, task_due_date))
    conn.commit()
...
def edit_task_data(new_task,new_task_status,new_task_date,task,task_status,task_due_date):
    c.execute(
        '''UPDATE taskstable SET task=?, task_status=?, task_due_date=? WHERE task=? and task_status=? and task_due_date=?''',
